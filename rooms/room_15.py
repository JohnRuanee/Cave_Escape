...
class Room():
    def __init__(self, door_num, screen, player):
        ...
        if self.door == 0:
            ...
            self.player_spawn = [(21.4 * tile, 7.6 * tile)]

        self.falling_walls = self.falling_walls_generate()

        self.doors = [door.Door((10 * tile, 13.8 * tile), 'down'), door.Door((22 * tile, 6 * tile), 'left')]
    ...
    def generate(self, player):
        ...
        self.entities.append(bug.Bug((4 * tile, 4 * tile), self.screen))
        self.entities.append(bug.Bug((4 * tile, 8 * tile), self.screen))
        self.entities.append(bug.Bug((4 * tile, 12 * tile), self.screen))

        self.entities.append(bug.Bug((8 * tile, 4 * tile), self.screen))
        self.entities.append(bug.Bug((8 * tile, 8 * tile), self.screen))
        self.entities.append(bug.Bug((8 * tile, 12 * tile), self.screen))

        self.entities.append(bug.Bug((12 * tile, 4 * tile), self.screen))

        self.entities.append(bug.Bug((16 * tile, 4 * tile), self.screen))
        self.entities.append(bug.Bug((16 * tile, 12 * tile), self.screen))

        self.entities.append(bug.Bug((20 * tile, 4 * tile), self.screen))
        self.entities.append(bug.Bug((20 * tile, 12 * tile), self.screen))
